...
def affine_backward(dout, cache):
    ...
    dx, dw, db = dout.dot(w.T), x_flattened.T.dot(dout), np.sum(dout,axis=0)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    ##formula from https://kevinzakka.github.io/2016/09/14/batch_normalization/
...
def batchnorm_backward_alt(dout, cache):
    ...
    N = dout.shape[0]
    dx = (inv_var/N) * (N*dxhat - np.sum(dxhat, axis=0) - x_hat*np.sum(dxhat*x_hat, axis=0))
...
def dropout_backward(dout, cache):
    ...
    return  dout if mode=="test" else dout * mask
        ...
        for f in range(F):
            for i in range(out.shape[2]):
                for j in range(out.shape[3]):
                    i2, j2 = i*stride, j*stride

                    out[n,f,i,j] = np.sum(w[f] * x_padded[n][:,i2:i2+HH,j2:j2+WW])
            out[n,f] += b[f]
        ...
        for f in range(F): 
            for i in range(dout.shape[2]):
                for j in range(dout.shape[3]):
                    ...
                    dw[f] += x_padded[n,:, i2:i2+HH, j2:j2+WW] * dout[n, f, i, j]


                    dx_padded[n,:,i2:i2+HH, j2:j2+WW] += w[f] * dout[n, f, i, j]
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H2):
                for j in range(W2):
                    i2, j2 = i*stride, j*stride
                    window = x[n,c,i2:i2+pool_height, j2:j2+pool_width]
        ...
        for c in range(C):
            for i in range(H2):
                for j in range(W2):
                    ...
                    dx[n,c,i2:i2+pool_height, j2:j2+pool_width] = dout[n,c,i,j]
                    dx[n,c,i2:i2+pool_height, j2:j2+pool_width][window!=np.max(window)] = 0
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    ## code from https://stackoverflow.com/questions/71727162/why-transpose-in-spatial-batch-normalization
